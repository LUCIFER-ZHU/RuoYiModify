  ...
  <div class="component-upload-image">
    <div class="upload-image-list">
      ...
      <div v-if="fileList.length > 0" class="custom-file-list">
        <div v-for="(file, index) in fileList" :key="index" class="custom-file-item">
          ...
          <div v-else class="image-file-item" @click.stop="handlePictureCardPreview(file)">
            <el-image :src="file.url" :alt="file.name" class="file-preview" fit="cover" />
          </div>
    ...
    <Teleport to="body">
      ...
      <el-image-viewer
        ...
        @close="closeViewer"
      />
      <div v-if="showImageViewer && signatureUrl" class="watermark-overlay-fixed" :style="{ backgroundImage: `url(${signatureUrl})` }"></div>
    </Teleport>
  </div>
</template>
...
import { getSignatureDetail } from '@/api/Website/electronicSignature';
...
onMounted(async () => {
  try {
    const res = await getSignatureDetail();
    if (res && res.data && res.data.url) {
      signatureUrl.value = res.data.url;
    }
  } catch (e) {
