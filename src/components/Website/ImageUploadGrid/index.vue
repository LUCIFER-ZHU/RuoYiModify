...
function handleUploadSuccess(res, file) {
  ...
  if (res.code === 200) {
    ...
    if (typeof url === "string") {
      ...
      if (tempUrl.indexOf(baseUrl) === -1 && !isExternal(tempUrl)) {
        uploadList.value.push({ name: baseUrl + tempUrl, url: baseUrl + tempUrl });
      } else {
        uploadList.value.push({ name: tempUrl, url: tempUrl });
      }
    }
